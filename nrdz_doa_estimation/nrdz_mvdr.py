...
def main():
    ...
    if n_ants == 5:
        ...
        rx = np.zeros(n_ants)
        rx[0] = 0
        rx[1] = 1*(c/freq)/2 #r_rooftop[0]/coordinate_scaling
        rx[2] = 1*(c/freq)/2 #r_gate[0]/coordinate_scaling
        rx[3] = -1*(c/freq)/2 #r_chime[0]/coordinate_scaling
        rx[4] = -1*(c/freq)/2 #r_west[0]/coordinate_scaling
        rx[5] = 1*(c/freq)/2 
        rx[6] = -1*(c/freq)/2
        rx[7] = 0 
        rx[8] = 0 
        ...
        ry = np.zeros(n_ants)
        ry[0] = 0
        ry[1] = 1*(c/freq)/2 #r_rooftop[1]/coordinate_scaling
        ry[2] = -1*(c/freq)/2 #r_gate[1]/coordinate_scaling
        ry[3] = 1*(c/freq)/2 #r_chime[1]/coordinate_scaling
        ry[4] = -1*(c/freq)/2 #r_west[1]/coordinate_scaling
        ry[5] = 0 
        ry[6] = 0
        ry[7] = 1*(c/freq)/2 
        ry[8] = -1*(c/freq)/2  
    ...
    plt.figure()
    plt.imshow(P[0:deg_range,0:deg_range], origin="lower", extent=[0, deg_range, 0, deg_range], aspect='auto', interpolation='none')
    ...
    # Plot map of power estimate at a particular coarse channel
    plt.figure()
    plt.plot(P[0:deg_range,src1_az])
    plt.title("Power spectrum over elevation at Az = "+str(src1_az)+" deg")
    ...
    # Plot map of power estimate at a particular coarse channel
    plt.figure()
    plt.plot(P[0:deg_range,(src1_az+ang_spread)])
    plt.title("Power spectrum over elevation at Az = "+str(src1_az+ang_spread)+" deg")
    plt.ylabel("Power (arb. units)")
    plt.xlabel("Elevation")
    plt.show()
