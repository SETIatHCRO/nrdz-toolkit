...
ang_spread_az = 23 # 0.09 # The angle spread between sources in azimuth
...
def main():
    ...
    array_flag = 2 # Flag to run either current nrdz sensor array or ideal nrdz sensor array
    ...
    if array_flag == 0:
        ...
        for x in range(0,int(antenna_array_dimension)):
            for y in range(0,int(antenna_array_dimension)):
                rx[y + (x*int(antenna_array_dimension))] = (x-(antenna_array_dimension/2))*(wavelength/2)*scale_baseline
                ry[y + (x*int(antenna_array_dimension))] = (y-(antenna_array_dimension/2))*(wavelength/2)*scale_baseline
    ...
    #synthesized_beam = np.zeros([deg_range,1])
    ...
    for c in range(0, n_points):
        ...
        for az in range(0,deg_range):
            ...
            #synthesized_beam[az,0] = np.square(abs(beam_coordinate[0]))
    ...
    #plt.figure()
    #plt.plot(az_range, synthesized_beam[0:deg_range])
    #plt.title("Synthesized beam (dirty beam) over elevation at El = " + str(el) + " deg")
    #plt.ylabel("Power (dB)")
