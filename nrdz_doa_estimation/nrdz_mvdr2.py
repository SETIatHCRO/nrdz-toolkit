...
def main():
    ...
    P_est = np.zeros([deg_range,deg_range])
    ...
    for el in range(0,deg_range):
        for az in range(0,deg_range):
            ...
            clean_image[el,az] = ((0.2e16)/P_est[el,az])
    ...
    plt.figure()
    plt.imshow(np.log(P_est[0:deg_range,0:deg_range]), origin="lower", extent=[min(el_range), max(el_range), min(az_range), max(az_range)], aspect='auto', interpolation='none')
    plt.title("MVDR power estimate over elevation and azimuth (dB)")
    plt.ylabel("Elevation (degrees)")
    plt.xlabel("Azimuth (degrees)")
    plt.show()
    
    plt.figure()
    plt.imshow(P_est[0:deg_range,0:deg_range], origin="lower", extent=[min(el_range), max(el_range), min(az_range), max(az_range)], aspect='auto', interpolation='none')
    plt.title("MVDR power estimate over elevation and azimuth")
    plt.ylabel("Elevation (degrees)")
    plt.xlabel("Azimuth (degrees)")
    plt.show()
    ...
    # Plot map of power estimate at a particular coarse channel
    plt.figure()
    plt.plot(az_range, P_est[0:deg_range,az1_idx[0]])
    plt.title("MVDR power estimate over elevation at Az = " + str(az_range[az1_idx[0]]) + " deg")
    plt.ylabel("Power (arb. units)")
    plt.xlabel("Elevation (degrees)")
    plt.show()
    
    plt.figure()
    plt.plot(az_range, P_est[0:deg_range,az2_idx[0]])
    plt.title("MVDR power estimate over elevation at Az = " + str(az_range[az2_idx[0]]) + " deg")
    plt.ylabel("Power (arb. units)")
    plt.xlabel("Elevation (degrees)")
    plt.show()
