...
def main():
    ...
    el_rad = el*np.pi/180
    az_2_closest_est = np.zeros(2)
    per_baseline_2_closest_az = np.array([])
        ...
        for n in range(0,n_ants):
            for m in range(0,n_ants): 
                if n!=m:
                    ...
                    tau = np.asarray([(-1/c)*(((rx[n]-rx[m])*np.sin(el_rad)*np.cos(az_rad[az]) + (ry[n]-ry[m])*np.sin(el_rad)*np.sin(az_rad[az]) + (rz[n]-rz[m])*np.cos(el_rad))) for az in range(0,deg_range)])
                    
                    az_closest_indices = np.argsort(abs(tau-calc_delay))
                    closest_indices = az_closest_indices[:2]
                    az_2_closest = az_rad[closest_indices]*180/np.pi
                    
                    per_baseline_2_closest_az = np.concatenate((per_baseline_2_closest_az,az_2_closest),axis=0)

                    print("The 2 closest az values = " + str(az_2_closest))

    #np.savetxt("tau.txt", tau)
    
    unique_vals,counts = np.unique(per_baseline_2_closest_az.astype(int), return_counts=True)
    max_count_idx = np.argmax(counts)
    est_az_angle_tdoa_tmp = unique_vals[counts==counts[max_count_idx]]
    ...
    axis_limit = 1200
    plt.figure()
    plt.plot(rfi_radius*np.cos(est_az_angle_tdoa*np.pi/180),rfi_radius*np.sin(est_az_angle_tdoa*np.pi/180), 'ro', label='TDOA - '+str(np.round(est_az_angle_tdoa))+'$^\circ$')
